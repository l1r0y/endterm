...
class CustomerAction implements UserAction {
    ...
    private void checkout(Connection connection) throws SQLException {
        int totalPrice = calculateTotalPrice(connection);
        System.out.println("Total price of items in cart: $" + totalPrice);
    }

    private int calculateTotalPrice(Connection connection) throws SQLException {
        int totalPrice = 0;
        String query = "SELECT price FROM clothes WHERE type = ?";
        PreparedStatement preparedStatement = connection.prepareStatement(query);
        for (String item : cart) {
